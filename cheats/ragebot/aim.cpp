...
void aim::fire(CUserCmd* cmd)
{
	...
	log << crypt_str("Shot at: ") + (std::string)player_info.szName + crypt_str(": ");
	...
	log << crypt_str("Hitchance: ") + (final_hitchance == 101 ? crypt_str("Auto") : std::to_string(final_hitchance)) + crypt_str(", ");
	log << crypt_str("BackTrack: ") + std::to_string(backtrack_ticks) + crypt_str("");

	if (g_cfg.misc.logs_enabled)
